...
def preprocess_text(text):
    ...
    doc = nlp(normalized_text)
    tokens = [
        token.lemma_.lower().replace("___", "\'/.,:;?!&*()_\-=+#|{[}]") if "___" in token.lemma_ else token.lemma_.lower()
        ...
        for title, folder_path in pdf_files.get(selected_category, []):
            if os.path.isdir(folder_path):
                for file_name in os.listdir(folder_path):
                    if file_name.lower().endswith(".pdf"):
                        pdf_path = os.path.join(folder_path, file_name)
                        results += search_in_pdf(pdf_path, corrected_search_text)
            elif folder_path.lower().endswith(".pdf"):
                results += search_in_pdf(folder_path, corrected_search_text)
        return results
    except Exception as e:
        return []

def search_in_pdf(pdf_path, corrected_search_text):
    results = []
    try:
        pdf_document = fitz.open(pdf_path)
        for page_num in range(len(pdf_document)):
...
def show_main_content():
    ...
    if selected_category:
        ...
        if st.button("Rechercher") and search_text:
            processed_text, corrected_text = preprocess_text(search_text)
            ...
            if results:
                ...
                for i, (pdf_path, page_num, paragraph) in enumerate(results, start=1):
                    file_name = os.path.basename(pdf_path)
                    with st.expander(f"Fichier : {file_name}, Page : {page_num}"):
                        st.write(paragraph)
                        img_data = extract_page(pdf_path, page_num, paragraph)
                        st.image(img_data, caption=f"Page {page_num} de {file_name}")
                        with open(pdf_path, "rb") as pdf_file:
